...
bool CXMLParser::load(bool is_fileParseEnable)
{
	...
	if (!root)
	{
		mLoggerIns->elog("CXMLParser::CXMLParser", root_name + " rss xml not valid.");
		return false;
	}
...
bool CXMLParser::get_same_tag(tinyxml2::XMLElement* base_element, string _tag, vector<string>& _output)
{
	...
	do
	{
		if (base_element->GetText())
			_output.push_back(base_element->GetText());
...
bool CXMLParser::save(string _file_path)
{
	tinyxml2::XMLError error = doc.SaveFile(_file_path.c_str());
	if (error != tinyxml2::XML_SUCCESS)
	{
		return false;
	}
